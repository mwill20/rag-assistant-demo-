...
class Settings:
    ...
    SYSTEM_PROMPT_PATH: Path = field(
        default_factory=lambda: Path(_env("SYSTEM_PROMPT_PATH", ""))
    )
    ...
    RETRIEVAL_MODE: str = field(
        default_factory=lambda: _env("RETRIEVAL_MODE", "knn").lower()
    )  # "knn" or "mmr"
    K: int = field(default_factory=lambda: int(_env("K", "3")))

    # LLM selection (preferences are parsed from this string)
    LLM_PREFERENCE_STR: str = field(
        default_factory=lambda: _env("LLM_PREFERENCE", "openai,groq,gemini")
    )
    OPENAI_MODEL: str = field(
        default_factory=lambda: _env("OPENAI_MODEL", "gpt-4o-mini")
    )
    GROQ_MODEL: str = field(
        default_factory=lambda: _env("GROQ_MODEL", "llama3-70b-8192")
    )
    GEMINI_MODEL: str = field(
        default_factory=lambda: _env("GEMINI_MODEL", "gemini-1.5-pro")
    )
    ...
    # Output
    DEFAULT_RETURN_FORMAT: str = field(
        default_factory=lambda: _env("DEFAULT_RETURN_FORMAT", "text").lower()
    )  # "text" or "json"
    ...
    def __post_init__(self):
        ...
        if not str(self.SYSTEM_PROMPT_PATH):
            object.__setattr__(
                self,
                "SYSTEM_PROMPT_PATH",
                self.ROOT / "src" / "rag_assistant" / "prompts" / "system_prompt.txt",
            )
